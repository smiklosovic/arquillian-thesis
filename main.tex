...
\newpage
\thispagestyle{empty}
\begin{textblock*}{\paperwidth}(8mm,0mm)
...
The extreme way of programming is also about doing plans but the strongest attitude can be sum up with the slogan "Do the Simplest Thing that Could Possibly Work" and the abbreviation \textit{YAGNI}\cite{bib011} which stands for "You Are not Going to Need It".
...
\lstinputlisting[language=Java,label=pop-pipe,caption=Modification of AndroidManifest.xml of Selendroid server]{src/selendroid_manifest.java}
...
\lstinputlisting[language=Java,label=pop-pipe,caption=Native testing via WebDriver]{src/selendroid-test.java}
