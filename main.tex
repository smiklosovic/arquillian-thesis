...
Regarding web applications, it is clear that they are not going to be run on Android device itself. Web applications are deployed in containers as we are used. However, when executing the tests, web application or its part from \texttt{@Deployment} has to be deployed in web container but tests have to be executed from Android point of view, in browser which runs on Android device, against the web deployment.
...
Arquillian Android container has to be \textit{testing agnostic} meaning the decision about the way of testing (web or native mode) is carried out upon run time. Whichever implementation of testing mode will be present on class path during testing process, that testing mode will be used.
...
\chapter{Implementation of Arquillian Android container}
	...
	\subsection{Usage}
		...
		\subsubsection{Mixed container adapters}
	
		\subsubsection{Multiple Android containers}

\section{Support for WebDriver}

\section{Support for native testing}
